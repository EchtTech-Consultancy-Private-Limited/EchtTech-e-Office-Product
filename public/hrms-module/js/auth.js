...
function handleVerificationResponse(response) {
    const errorText = $(".verify_otp_error");

    switch (response.status) {
        case 1:
            hideElement("otp");
            showElement("user_password");
            $('#password_email').val(response.userDetail.email);
            $('#password_mob_number').val(response.userDetail.mobile);
            $('#password_user_role').val(response.userDetail.userRole);
            break;
        case 0:
            errorText.text("Please enter a valid verification code.");
            break;
        case 2:
            errorText.text("Your OTP has expired.");
            break;
        default:
